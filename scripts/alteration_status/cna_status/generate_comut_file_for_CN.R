...
load("tRCC_allSamples_test_again_geneMats.RData") #1=LOH and 0=notLOH
...
ploidy <- read.csv("../../../metadata/tRCC_allSamples_optimals.csv")[,c("sample","category","value")]

#table(as.character(ploidy$sample)==samples)

#load gene list (CGC tier1 gene list, 571 genes)
gene_dat <- read.table("../../../metadata/tier1_gene_coordinates.txt", sep="\t", head=T)
...
write.csv(out_alteration_all, sprintf("comut_CN_tRCC_allSamples_optimals.csv", cp_threshold), quote=F)
